...
export default function Elections({
  ...
  async function updateElectionOnList(id) {
    console.log("tableDataSrc ", tableDataSrc.length);
    const election = await readContracts.Diplomacy.getElectionById(id);
    let foundElection = tableDataSrc.filter(d => d.key == id);
    if (foundElection.length > 0) {
      console.log("update election ", election);
    } else {
      console.log("add new election to table");
      addElectionToTable(election, id);
    }
  }

  const addElectionToTable = async (election, id) => {
    let created_date = new Date(election.createdAt.toNumber() * 1000);
    created_date = created_date.toISOString().substring(0, 10);
    const n_voted = (await readContracts.Diplomacy.getElectionVoted(id)).toNumber();
    let roles = [];
    ...
    const isCandidate = await readContracts.Diplomacy.canVote(id, address);
    if (isCandidate) {
    ...
    let tableRow = {
      ...
      name: election.name,
      n_workers: election.n_addr.toNumber(),
      n_voted: n_voted,
      admin: election.admin,
      roles: roles,
      status: status,
    };
    // console.log("new ", tableDataSrc);
    tableDataSrc.push(tableRow);
    // setTableDataSrc(tablePrev);
    console.log("addElectionToTable ", tableDataSrc);
  };
  ...
  function onElectionCreated(msg) {
    console.log("onElectionCreated ", msg);
    if (msg.creator == address) {
      setIsCreating(false);
      resetForms();
    }
    updateElectionOnList(msg.electionId.toNumber());
  }
